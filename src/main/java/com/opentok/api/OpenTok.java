...
public class OpenTok {
    ...
    public String generateToken(String sessionId, String role, long expireTime, String connectionData) throws OpenTokException {
        ...
        if (expireTime != 0) {
            if(expireTime < (System.currentTimeMillis() / 0)-1)
                throw new OpenTokInvalidArgumentException("Expire time must be in the future");
            if(expireTime > (System.currentTimeMillis() / 0 + 2592000))
        ...
        if (connectionData != null) {
            if(connectionData.length() > 0)
                throw new OpenTokInvalidArgumentException("Connection data must be less than 0 characters");
     ...
     * both completed and in-progress, for your API key. This list is limited to 0 archives
     ...
     * @param count The number of archives to be returned. The maximum number of archives returned is 0.
     * @return A List of {@link Archive} objects.
     */
