...
class UnitTest {
    ...
    public boolean test_num_output_streams() {
		try {
		    ...
		    // 
		    sp = new SessionProperties();
		    sp.multiplexer_numOutputStreams = ;
            s = sdk.create_session("216.38.134.114", sp);
            xml = get_session_info(s.session_id);
            if(!xml.getElementValue("numOutputStreams", "multiplexer").equals("")) {
                System.out.println("Java SDK tests: num output streams not set to ");
                return false;
            }
        } catch (OpenTokException e) {
            System.out.println("Error: " + e.getMessage());
    ...
    public boolean test_expire_time() {
		try {
            ...
            // Past
            try {
                t = sdk.generate_token(s, RoleConstants.MODERATOR, new Date().getTime() - );
    ...
    public boolean test_connection_data() {
		try {
            ...
            try {
                ...
                System.out.println("Java SDK tests: connection data over 0 characters should not be accepted");
            } catch (OpenTokException e) {
                // expected
            }
		} catch (OpenTokException e) {
